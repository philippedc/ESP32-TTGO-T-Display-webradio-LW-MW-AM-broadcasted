...
-> V2 gives separate output for audio (webradio) and carrier RF. That allows:
    - to transmit in MW range frequencies like 1500kHz, the highest RF frequency is, the best propagation is, for a same lenght of antenna
    - to consideraly increase power transmission with a power supply larger than 3.3V
...
v2.0  10 jan 26   - separate audio and RF output 

*/

// parameters
//-----------------
const String VERSION = "v2.0";     // code version
unsigned int rfFreq = 1500;        // carrier frequency in kHz
const char* dnshost = "emetteur";  // DNS name
...
#define PWM_Res       2     // [0 - 3] 2 bits resolution - choise = [1 - 16] 
...
//---------------
  ...
  rfFreq = pref.getUInt("freq", rfFreq);
...
//-------------------------------
  ...
  server.on("/freqplus", HTTP_GET, [](AsyncWebServerRequest *request) {
    rfFreq += 100;
    if( rfFreq > 1800 ) rfFreq = 1800;
    pref.putUInt("freq", rfFreq);
    RfTransmitter();
    change = true;
    request->redirect("/");
  });
...
// check station change
  if(!digitalRead(pb1))      {ChangeRadio(LOW); delay(500);}
  else if(!digitalRead(pb2)) {ChangeRadio(HIGH); delay(500);}

// display rfLevel 
  int rfLevel = analogRead(rfLevelPin);
  String legende = "Niv. Ant.: " + String(rfLevel);
  tft.fillRect(130, 30, 200, 20, backgroundColor);  // full screen resolution 135 x 240 pixels.
  tft.setCursor(5, 30);
  tft.setTextSize(2);
  tft.print(legende);
...
void DrawDisplay() {
  ...
  tft.setTextSize(2);
  String legende = "eRadio / PO " + String(rfFreq) + "kHz";
  tft.drawString(legende, tft.width()/2, tft.height()/2 -50);
  tft.drawString(radioName, tft.width()/2, tft.height()/2 +10 );
  tft.setTextSize(1);
  tft.drawString(radioTitle, tft.width()/2, tft.height()/2 +30 );
  legende = "Param:" + dnsName;
  tft.drawString(legende,   tft.width()/2, tft.height()/2 +50);
}
